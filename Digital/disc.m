...
% figure;
% p1_obs_cont = [-200 -180 -100+40i -100-40i];
% p1_ctr_cont = [-55 -45 -25+10i -25-10i];
% % p1_ctr_cont = [-52 -54 -14+14.2*1i -14-14.2*1i];
% 
% observable = rank([plant_disc.C; ...
%     plant_disc.C*plant_disc.A; ...
%     plant_disc.C*plant_disc.A*plant_disc.A; ...
%     plant_disc.C*plant_disc.A*plant_disc.A*plant_disc.A]') == 4
% 
% [K_obs, L_obs, observed_tracking] = observer_placement(plant_disc, p1_ctr_cont, p1_obs_cont, 0.5*sampling_period);
% [K_obs_drate, L_obs_rate, observed_tracking_drate] = observer_placement(plant_disc, p2_cont, p1_obs_cont, 0.5*sampling_period);
% 
% 
% inital_state = [1 15 115 155];
% x0 = [inital_state, -inital_state];
% observer_config = RespConfig("InitialState", x0);
% tracking_config = RespConfig("InitialState", inital_state);
% 
% [obs_y, obs_t] = step(observed_tracking2(1), observer_config);
% observer_stepinfo = stepinfo(obs_y, obs_t);
% 
% [trk_y, trk_t] = step(tracking2(1), tracking_config);
% tracker_stepinfo = stepinfo(trk_y, trk_t);
% 
% [obs_y2, obs_t2] = step(observed_tracking_drate(1), observer_config);
% observer_stepinfo_drate = stepinfo(obs_y2, obs_t2);
% 
% [trk_y2, trk_t2] = step(tracking4(1), tracking_config);
% tracker_stepinfo_drate = stepinfo(trk_y2, trk_t2);
% 
% 
% [p1y1, p1t1] = zoh(obs_y, obs_t);
% [p1y2, p1t2] = zoh(trk_y, trk_t);
% [p1y3, p1t3] = zoh(obs_y2, obs_t2);
% [p1y4, p1t4] = zoh(trk_y2, trk_t2);
% 
% hold on
% plot(p1t1, p1y1, 'DisplayName', 'Observer feedback')
% plot(p1t2, p1y2, 'DisplayName', "State feedback")
% plot(p1t3, p1y3, 'DisplayName', "Double rate output feedback")
% plot(p1t4, p1y4, 'DisplayName', "Double rate sate feedback")
% legend
% hold off
