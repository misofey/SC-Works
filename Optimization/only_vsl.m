...
initial_guess = repmat(60, 120, 1);
objective = @(x) the_goal(x, pars, q0, x0);

options = optimoptions('fmincon', 'Display', 'iter', 'Algorithm', 'interior-point','OptimalityTolerance', 1e-2);
...
function [x_new] = state_update(x_old, pars, u, q0, k)
    ...
    % Speed update
    x_new(6) = v(1) + ( pars("Tovertau") * (bigv(1) - v(1)) ) + - ant(1);
...
function [x] = simulate(pars, q0, u, x0)
    ...
    x = zeros(pars("nt"), 12);  % array that holds all 11 states and 1 input
    x(1, 1:11) = state_update(x0, pars, u(1), q0, 1);
    x(:, 12) = u;

    % Perform state update
    for i = 2:pars("nt")
        x(i, 1:11) = state_update(x(i-1, 1:11), pars, x(i, 12), q0, i-1);
    end
end
