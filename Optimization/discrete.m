...
u_results = ga(objective, 240, [],[], [],[], lb ,ub , nonlcon, intcon, options);
...
function [x] = simulate(pars, q0, u, x0)
    ...
    x(1, 1:11) = state_update(x0, pars, [u(1) u(121)], q0, 1);
    x(:, 12) = u(1:120) * 20;
    x(:, 13) = u(121:240) / 5;
...
function [c, ceq] = cons_fnc(u, pars, q0, initial_state)
    x = simulate(pars, q0, u, initial_state);
    c = x(:, 1:5) - pars("rhoc");
    % ceq = 0.0001*[cos(u(:, 1) * pi / 20), cos(u(:, 2) * pi / 0.2)];
    ceq = [];
end
